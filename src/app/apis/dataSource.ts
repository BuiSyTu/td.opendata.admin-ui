import { DataSource, DataSourceListFilter } from 'src/app/models'
...
const getAll = async (listFilter?: DataSourceListFilter) => {
    try {
...
const add = async (data: DataSource) => {
    try {
        const res = await axios({
            method: 'POST',
            headers: {
...
const update = async (id: string, data: DataSource) => {
    try {
        const res = await axios({
            method: 'PUT',
            url: `${baseUrl}/${id}`,
