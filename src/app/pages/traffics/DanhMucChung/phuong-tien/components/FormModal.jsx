...
import { HOST_API, requestGET, requestPOST, requestPUT, HOST_FILE, requestDELETE } from '../../../../../utils/basicAPI';
...
const ModalCategory = (props) => {
  ...
  const handleDelete = async (id) => {
    var res = await requestDELETE(`${HOST_API}/vehicle/${id}`);
    if (res) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res?.data) {
          setData(res.data);
          var temp = res.data.images.split('##');
          setImagesList(temp.length > 0 ? temp : []);
          let _data = res.data;
          let listCarUtility = []
        ...
        <Form layout='vertical' labelAlign='left' form={form}>
          <div className='row'>
            ...
            <div className='col-4'>
              ...
              >
                <Row>
                  {data && data.image && <Image width="70%" height={150} src={!(data.image.includes('https://') || data.image.includes('http://')) ? `${HOST_FILE + data.image}` : data.image} style={{ padding: '5%' }} />}
                </Row>
              </Form.Item>
            </div>
            <div className='col-4'>
              ...
              >
                <Row>
                  {imagesList &&
                    imagesList.length > 0 &&
                    imagesList.map((item) => <Image width={150} height={150} src={!(item.includes('https://') || item.includes('http://')) ? `${HOST_FILE + item}` : item} style={{ padding: '5%' }} />)}
                </Row>
              </Form.Item>
            </div>
