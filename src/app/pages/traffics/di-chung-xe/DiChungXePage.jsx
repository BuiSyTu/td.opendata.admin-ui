...
const CustomPage = () => {
  ...
  const columns = [
    ...
    {
      title: 'Nơi đi',
      dataIndex: 'placeDepartureName',
      key: 'code',
      width: '20%',
    },
    {
      title: 'Nơi đến',
      dataIndex: 'placeArrivalName',
      key: 'code',
      width: '20%',
    },
    {
    ...
    const fetchData = async () => {
      try {
        ...
        var res = await requestGET(
          `${HOST_API}/carpool?keySearch=${inputValue}&pageNumber=${offset}&&pageSize=${size}`
        );
  ...
  const handleDelete = async (id) => {
    var res = await requestDELETE(`${HOST_API}/carpool/${id}`);
    if (res) {
  ...
  return (
    <div>
      <PageTitle breadcrumbs={[]}>Danh mục chính sách xe</PageTitle>
