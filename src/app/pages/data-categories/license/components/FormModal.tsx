...
const ModalCategory = (props: any) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            const [status, res] = await licenseApi.getById(modalId)
            if (status === 200) {
    ...
    const postData = async (data: License) => {
        setButtonLoading(true)
        const [status] = await licenseApi.add(data)
        if (status === 200) {
    ...
    const putData = async (data: License) => {
        setButtonLoading(true)
        const [status] = await licenseApi.update(modalId, data)
        if (status === 200) {
            notification.success({
