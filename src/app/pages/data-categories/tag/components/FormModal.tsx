...
const ModalCategory = (props: any) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            const [status, res] = await tagApi.getById(modalId)
            if (status === 200) {
    ...
    const postData = async (data: Tag) => {
        setButtonLoading(true)
        const [status] = await tagApi.add(data)
        if (status === 200) {
    ...
    const putData = async (data: Tag) => {
        setButtonLoading(true)
        const [status] = await tagApi.update(modalId, data)
        if (status === 200) {
            notification.success({
