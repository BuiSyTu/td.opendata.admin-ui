...
import { organizationApi } from 'src/app/apis'
...
const ModalCategory = (props: any) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            const [status, res] = await organizationApi.getById(modalId)
            if (status === 200) {
    ...
    const postData = async (data: Organization) => {
        setButtonLoading(true)
        const [status] = await organizationApi.add(data)
        if (status === 200) {
    ...
    const putData = async (data: Organization) => {
        setButtonLoading(true)
        const [status] = await organizationApi.update(modalId, data)
        if (status === 200) {
            notification.success({
