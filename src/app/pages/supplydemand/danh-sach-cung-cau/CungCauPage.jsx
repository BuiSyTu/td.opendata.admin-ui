...
import { FaEdit, FaTrashAlt, FaFileAlt, FaEye } from '../../../../../node_modules/react-icons/fa';
...
const chatBreadCrumbs = [
  {
    title: 'Cung cầu',
    path: '/supplydemand/supplydemandlist',
    isSeparator: false,
    isActive: false,
  },
  {
...
const CungCauPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      var res = await requestGET(`${HOST_API}/Product?keySearch=${searchText}&pageNumber=${offset}&pageSize=${size}`)
  ...
  const handleDeleteModalData = async (id) => {
    var res = await requestDELETE(`${HOST_API}/Product/${id}`);
    if (res) {
  ...
  return (
    <Switch>
      <Route path='/supplydemand/supplydemandlist'>
