...
const MiningSourcePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const [status, res] = await miningSourceApi.getAll()
            ...
            if (status === 200) {
                ...
                notification.error({
                    message: 'Thất bại',
                    description: 'Có lỗi xảy ra trong quá trình thực hiện!',
                })
            }
    ...
    const handleDelete = async (id: string) => {
        const [status] = await miningSourceApi.delete(id)
        if (status === 200) {
    ...
    return (
        <div>
            <PageTitle breadcrumbs={[]}>Danh sách nguồn khai thác</PageTitle>
...
export default MiningSourcePage
