...
import { miningSourceApi } from 'src/app/apis'
...
const ModalCategory: React.FC<Props> = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            const [status, res] = await miningSourceApi.getById(modalId)
            if (status === 200) {
                form.setFieldsValue(res?.data)
            } else {
                notification.error({
                    message: 'Thất bại',
                    description: 'Vui lòng kiểm tra lại quá trình xử lý!',
                })
            }
            setIsLoading(false)
        }
    ...
    const postData = async (data: MiningSource) => {
        setButtonLoading(true)
        const [status] = await miningSourceApi.add(data)
        if (status === 200) {
    ...
    const putData = async (data: MiningSource) => {
        setButtonLoading(true)
        const [status] = await miningSourceApi.update(modalId, data)
        if (status === 200) {
            notification.success({
