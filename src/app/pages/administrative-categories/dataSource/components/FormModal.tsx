...
const ModalCategory = (props: CategoryProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        var res = await dataSourceApi.getById(modalId)
        if (res?.data) {
  ...
  const postData = async (data: DataSource) => {
    try {
      setButtonLoading(true)
      var res = await dataSourceApi.add(data)
      if (res) {
  ...
  const putData = async (data: DataSource) => {
    try {
      setButtonLoading(true)
      var res = await dataSourceApi.update(modalId, data)
      if (res) {
        notification.success({
  ...
  return (
    <Modal
      ...
      title={<Text style={{ fontWeight: '500', color: '#fff' }}>Loại giấy tờ</Text>}
      onOk={handleOk}
