...
const ModalCategory = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        var res = await requestGET(`${HOST_API}/JobPosition/${modalId}`);
        if (res?.data) {
  ...
  const postData = async (data) => {
    try {
      ...
      var res = await requestPOST(`${HOST_API}/JobPosition`, data);
      if (res) {
  ...
  const putData = async (data) => {
    try {
      setButtonLoading(true);
      var res = await requestPUT(`${HOST_API}/JobPosition/${modalId}`, data);
      if (res) {
        notification.success({
  ...
  return (
    <Modal
      ...
      title={<Text style={{fontWeight: '500', color: '#fff'}}>Vị trí tuyển dụng</Text>}
      //width='60%'
      onOk={handleOk}
