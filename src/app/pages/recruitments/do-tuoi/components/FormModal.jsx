...
const ModalCategory = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        var res = await requestGET(`${HOST_API}/JobAge/${modalId}`);
        if (res?.data) {
  ...
  const postData = async (data) => {
    try {
      ...
      var res = await requestPOST(`${HOST_API}/JobAge`, data);
      if (res) {
  ...
  const putData = async (data) => {
    try {
      setButtonLoading(true);
      var res = await requestPUT(`${HOST_API}/JobAge/${modalId}`, data);
      if (res) {
        notification.success({
  ...
  return (
    <Modal
      ...
      title={<Text style={{fontWeight: '500', color: '#fff'}}>Độ tuổi</Text>}
      //width='60%'
      onOk={handleOk}
