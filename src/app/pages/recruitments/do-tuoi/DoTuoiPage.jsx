...
const CustomPage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        var res = await requestGET(
          `${HOST_API}/JobAge?orderBy=name&keySearch=${inputValue}&pageNumber=${offset}&pageSize=${size}`
        );
  ...
  const handleDelete = async (id) => {
    var res = await requestDELETE(`${HOST_API}/JobAge/${id}`);
    if (res) {
  ...
  return (
    <div>
      <PageTitle breadcrumbs={[]}>Danh mục độ tuổi</PageTitle>
